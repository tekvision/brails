...
(function () {
    ...
    // See end of file for syntax examples

    window.setCalendar = function (pId, trigger, targ, commentsEnabled) {

        // Control the behavior of date selection clicks
        var handleClick = function (ev, dc) {
            targ.value = dc.range.wDays[dc.range.current.wDay].lng + ' ' + dc.range[dc.range.current.month].name + ' ' + dc.range.current.mDay + ', ' + dc.range.current.year;
            dc.close();
        }
      ,
        ...
        $A([
      {
         ...
         ,
          ...
          runBefore: function (dc) {             
              ...
              dc.source = '<table role="dialog" class="' + pId + '" aria-label="' + dc.role + '"><tr role="presentation"><td class="cal" accesskey="1" title="' + dc.prevTxt.replace(/<|>|\"/g, '') + ' ' + dc.yearTxt.replace(/<|>|\"/g, '') + '" role="button" id="' + dc.prevBtnId + 'Y" tabindex="0"><span>&#8656;</span></td><td title="' + dc.tooltipTxt.replace(/<|>|\"/g, '') + '" colspan="5" class="year"><span>' + dc.range.current.year + '</span></td><td class="cal" accesskey="2" title="' + dc.nextTxt.replace(/<|>|\"/g, '') + ' ' + dc.yearTxt.replace(/<|>|\"/g, '') + '" role="button" id="' + dc.nextBtnId + 'Y" tabindex="0"><span>&#8658;</span></td></tr><tr role="presentation"><td class="cal" accesskey="3" title="' + dc.prevTxt.replace(/<|>|\"/g, '') + ' ' + dc.monthTxt.replace(/<|>|\"/g, '') + '" role="button" id="' + dc.prevBtnId + '" tabindex="0"><span>&#8592;</span></td><td colspan="5" class="month"><span>' + dc.range[dc.range.current.month].name + '</span></td><td class="cal" accesskey="4" title="' + dc.nextTxt.replace(/<|>|\"/g, '') + ' ' + dc.monthTxt.replace(/<|>|\"/g, '') + '" role="button" id="' + dc.nextBtnId + '" tabindex="0"><span>&#8594;</span></td></tr><tr role="presentation">';
            ...
            commentsAll = dc.range[dc.range.current.month].comments['*'];
              for (var i = 1; i <= dc.range[dc.range.current.month].max;
            i++) {
                  dc.range.track[dc.baseId + i] = i;
                  m.setDate(i);
                  var dis = (disabled && inArray(i, disabled) !== -1) || (disabledAll && inArray(i, disabledAll) !== -1) ? true : false,
               comm = '';
                  ...
                  dc.source += i + ', ' + dc.range.wDays[m.getDay()].lng + ' ' + dc.range[dc.range.current.month].name + ' ' + dc.range.current.year;
         ...
         ,
          ...
          runAfter: function (dc) {
              ...
              $A.bind('td.day',
            {
               ...
               ,
                click: function (ev) {
                    ...
                    dc.setCurrent(dc);
                    if (hasClass(this, 'selected') || (!commentsEnabled && !hasClass(this, 'comment'))) {
                        if (inArray(dc.range.current.mDay, dc.range[dc.range.current.month].disabled[dc.range.current.year] || dc.range[dc.range.current.month].disabled['*'] || []) === -1) {
                            handleClick.apply(this, [ev, dc]);
                        }
                    }
                    else dc.setFocus(this, null, true);
                    ev.preventDefault();
                }
               ,
                ...
                keydown: function (ev) {
                    ...
                    if ((k >= 37 && k <= 40) || k == 27 || (k >= 33 && k <= 36)) {
                        ...
                        if (k == 37) {
                            ...
                            if (wd != dc.iterS && dc.range.current.mDay > 1) {
                                while (dc.range.current.wDay != dc.iterS && getEl(dc.baseId + (dc.range.current.mDay - 1))) {
                            ...
                            if (wd != dc.iterE && dc.range.current.mDay < dc.range[dc.range.current.month].max) {
                                while (dc.range.current.wDay != dc.iterE && getEl(dc.baseId + (dc.range.current.mDay + 1))) {
              ...
              $A.bind('#' + dc.nextBtnId + 'Y',
            {
              ...
              if (commentsEnabled) dc.children[1].open();
          }
         ,
        ...
        $A($A.reg[pId], [
      {
         ...
         ,
          ...
          openEditor: false,
          source: '<textarea style="visibility: hidden; display: none;" class="commentTa" title="Comment"></textarea><button title="Edit Comment" class="commentBtn">Edit</button>',
         ...
         ,
          ...
          runDuring: function (dc) {
              setAttr(dc.accDCObj,
            {
              ...
              setAttr(dc.containerDiv, 'role', 'presentation');
          }
         ,
          ...
          add: function (dc) {
            ...
            pre = '';
              if ((dc.parent.range[dc.parent.range.current.month].disabled[dc.parent.range.current.year] && inArray(dc.parent.range.current.mDay, dc.parent.range[dc.parent.range.current.month].disabled[dc.parent.range.current.year]) !== -1) || (dc.parent.range[dc.parent.range.current.month].disabled['*'] && inArray(dc.parent.range.current.mDay, dc.parent.range[dc.parent.range.current.month].disabled['*']) !== -1))
                  pre += dc.parent.disabledTxt.replace(/<|>|\"/g, '') + ' ';
              if (!comm) remClass(dc.parent.current, 'comment');
              else {
                  addClass(dc.parent.current, 'comment');
                  pre += dc.parent.commentedTxt.replace(/<|>|\"/g, '') + ' ';
              }
              lbl = pre + lbl;
              setAttr(dc.parent.current,
            {
                title: trim(pre),
         ...
         ,
          ...
          reset: function () {
              ...
              if (dc.openEditor) {
                  ...
                  setAttr(dc.textarea,
               {
                   title: 'Comment for ' + dc.parent.range.current.mDay + ', ' + dc.parent.range.wDays[dc.parent.range.current.wDay].lng + ' ' + dc.parent.range[dc.parent.range.current.month].name + ' ' + dc.parent.range.current.year
               }
               ).focus();
                  if (dc.comments[dc.parent.range.current.year] && dc.comments[dc.parent.range.current.year][dc.parent.range.current.mDay])
                      dc.textarea.value = dc.comments[dc.parent.range.current.year][dc.parent.range.current.mDay];
                  setAttr(dc.commentBtn,
               {
                   title: 'Save Comment'
               }
               ).innerHTML = 'Save';
